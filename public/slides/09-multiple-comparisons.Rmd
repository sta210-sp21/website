...
- Ex: There are 3 depth levels in our data, so we can make ${3 \choose 2} = 3(3-1)/2 = 3$ comparisons

---

## Comparisons for the aldrin data
...
- The probability of making an individual Type I error is $\alpha = 1 - C$, where $C$ is the confidence level 
...
- When we make mutliple comparisons, we will select the critical value $t^*$ to control for the probability of making a  **family-wise Type I error**
...
.vocab[Goal: ] Choose the critical value $t^*$ such that the probability of making a family-wise Type I error is $\alpha$.

To do so, we will choose $t^*$ such that the probability of making an indiviudal Type I error is $\alpha/m$, where $m$ is the number of comparisons 
...
We are making 3 comparisons. 

Therefore, we want probability of making an individual Type I error to be $\alpha / m = 0.05 / 3$. 
...
## Making pairwise comparisions in R
