...
1. Let's prepare the data for analysis and modeling.
...
1. Fit a model using mean-centered age, race, gender, income, and education to predict the odds that a person is a possible voter. Display the model.

1. *Should party identification be added to the model?* Use the appropriate test to determine if party identification should be added to the model. Include the hypotheses in mathematical notation, the output from the test, and the conclusion in the context of the data. 
...
1. *Lastly, let's use the model to identify possible voters!*  Let's suppose you want to use the model to identify possible voters to help a Get Out the Vote (GOTV) campaign target its efforts in the final weeks before an election. You decide to use a threshold of 0.58 to classify observations into possible voters vs. non-voters.
